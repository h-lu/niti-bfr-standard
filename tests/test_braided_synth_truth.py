...
def test_truth_metrics_use_body_only_axis_span_and_area_definition() -> None:
    ...
    assert expected_axis_span < float(curve_positions_px[-1])
    assert np.isclose(truth["length_axis_true_px"], expected_axis_span)
    assert np.isclose(truth["area_proj_true_px2"], expected_area)
