...
class AttachmentPruningTests(unittest.TestCase):
    def test_attachment_pruning_removes_border_rod_and_tip_cap(self) -> None:
        ...
        self.assertEqual(int(cleaned[30, 6]), 0)
        self.assertEqual(int(cleaned[45, 160]), 0)
        self.assertEqual(int(cleaned[45, 90]), 255)
        self.assertEqual(int(rejected[30, 6]), 255)
        self.assertEqual(int(rejected[45, 160]), 255)

    def test_attachment_pruning_keeps_long_aligned_residual_band(self) -> None:
        component = np.zeros((90, 180), dtype=np.uint8)
